...
class CircuitGrid(pygame.sprite.RenderPlain):
    ...
    def handle_input(self, key):
        # vsilva
        if key == pygame.K_a : self.move_to_adjacent_node(MOVE_LEFT)
        if key == pygame.K_d: self.move_to_adjacent_node(MOVE_RIGHT)
        if key == pygame.K_w: self.move_to_adjacent_node(MOVE_UP)
        if key ==  pygame.K_s: self.move_to_adjacent_node(MOVE_DOWN)
        if key ==  pygame.K_x: self.handle_input_x()
        if key ==   pygame.K_y: self.handle_input_y()
        if key == pygame.K_z: self.handle_input_z()
        if key == pygame.K_h: self.handle_input_h()
        if key ==     pygame.K_SPACE: self.handle_input_delete()
        if key ==     pygame.K_c: self.handle_input_ctrl()
        if key ==     pygame.K_UP: self.handle_input_move_ctrl(MOVE_UP)
        if key ==  pygame.K_DOWN: self.handle_input_move_ctrl(MOVE_DOWN)
        if key == pygame.K_LEFT: self.handle_input_rotate(-np.pi / 8)
        if key == pygame.K_RIGHT: self.handle_input_rotate(np.pi / 8)
        if key ==  pygame.K_m: self.handle_input_t() # vsilva slowdown
        if key ==  pygame.K_o: self.handle_input_speedup() # vsilva speedup
        if key ==  pygame.K_k: self.handle_input_pause() # vsilva pause
    ...
    # vsilva pause K
    def handle_input_pause(self):
        selected_node_gate_part = self.get_selected_node_gate_part()
        if selected_node_gate_part == node_types.EMPTY:
            circuit_grid_node = CircuitGridNode(node_types.K)
            self.model.set_node(self.selected_wire, self.selected_column, circuit_grid_node)
    ...
    def handle_input_t(self):
        ...
        selected_node_gate_part = self.get_selected_node_gate_part()
        if selected_node_gate_part == node_types.EMPTY:
            circuit_grid_node = CircuitGridNode(node_types.L)
            self.model.set_node(self.selected_wire, self.selected_column, circuit_grid_node)
        #slow down
        globals.TICKS = 3
    ...
    # vsilva speed up
    def handle_input_speedup(self):
        selected_node_gate_part = self.get_selected_node_gate_part()
        if selected_node_gate_part == node_types.EMPTY:
            circuit_grid_node = CircuitGridNode(node_types.O) #S)
            self.model.set_node(self.selected_wire, self.selected_column, circuit_grid_node)
...
class CircuitGridGate(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        # vsilva slowdown
        elif node_type == node_types.L:
            self.image, self.rect = resources.load_image('gates/minus_gate.png', -1)
        # vsilva speedup
        elif node_type == node_types.O:
            self.image, self.rect = resources.load_image('gates/plus_gate.png', -1)
        # vsilva pause
        elif node_type == node_types.K:
            self.image, self.rect = resources.load_image('gates/pause_gate.png', -1)
